...
{	
	for (size_t j = 0; j < 151; ++j) maskSSE[j] = _mm_set1_epi16(j*gap);
	for (size_t j = 0; j < 1208; ++j){
		maskSSE64[j] = _mm_set1_epi16((j/8)*gap);
	}
	for (size_t j = 0; j < 151; ++j) maskNormal[j] = j*gap;
	
}

/*
//das ist die Value-Funktion für NWTemp()
int16_t Alignment::getVal(const char a, const char b){
	return ((a==b) ? _match : _mismatch);
}

//lineares computen der scores mithilfe zweier Temp values
//diese Funktion wird nicht benutzt, sie zeigt nur, wie meine Implementierung für einen Vergleich aussieht
int Alignment::needleWunschTemp(const std::string& seq1, const std::string& seq2){
	int16_t linear[151];
	std::copy(std::begin(maskNormal), std::end(maskNormal), std::begin(linear));
	//for (size_t j = 0; j < 151; ++j) linear[j] = (j)*gap;

	//std::vector<int> linear;
	//linear.resize(seq2.size()+1, 0);
	...
	for (size_t i = 1; i <= seq1.size(); ++i){
		...
		for (size_t j = 1; j <= seq2.size(); ++j){
			diag = linear[j-1];
			linear[j-1] = oben;
			oben = std::max(diag+getVal(seq1[i-1],seq2[j-1]), std::max(linear[j]+_gap, linear[j-1]+_gap));
		}

		linear[seq2.size()] = oben;
	}
...
int Alignment::computeSSE(const std::string& s0, const int16_t(& v)[1200], const uint32_t number_cmp) {
	...
	//for (size_t j = 0; j < 1208; ++j) linear[j] = (j/8)*gap;
...
int Alignment::computeSSE64Blocks(const std::string& s0, __m128i (& v)[150], const __m128i (& w)[150]) {
	...
	for (size_t i = 1; i < 151; ++i)
	{
		...
		for (size_t m = 0; m<8; ++m, ++ptrd) *ptrd = oben[m];
	}
